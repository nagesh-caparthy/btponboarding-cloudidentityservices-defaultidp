...
Once the new users are created, imported, or updated, we need to run the manual or scheduled jobs to provide the users to make the applications accessible to the newly added users.  

This service provides the choice to self-register for specific application access in real-time. As an administrator, you can configure the target systems and provision users to these systems.  
